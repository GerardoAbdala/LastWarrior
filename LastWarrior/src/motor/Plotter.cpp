...
void Plotter::pintar_textura(SDL_Renderer *r,SDL_Texture *textura,SDL_Rect &src , SDL_Rect &dest, Coordenadas size)
{
    ...
    //SDL_RenderCopyEx(r,textura,&src,&dest,0,NULL,SDL_FLIP_VERTICAL);
};

void Plotter::invertir_textura(SDL_Renderer *r,SDL_Texture *textura,SDL_Rect &src , SDL_Rect &dest,Coordenadas size){
    SDL_QueryTexture(textura,NULL,NULL,NULL,NULL);
    dest.w=size.x;
    dest.h=size.y;
