    ...
    for(int i=0;i<columna;++i)
    {
        ...
        for(int j=0;j<renglon;++j)
        {
            ...
            TileInfo tile
            {
                ...
                {j*info.tile_width/*Para evitar errores de espacio sumar aqui pixeles de acuerdo al tamaño del espacio ejemplo +10*/,i*info.tile_height/*Aqui poner lo mismo pero espacio hacia abajo en Y*/,info.tile_width,info.tile_height},//dstRect
                atlas_sheet,//source
            ...
            if(info.mapa_ids[i][j]==5)
                tiene_colision = true;//(info.mapa_ids[i][j]==notidobjfisicos) ? false : true;
